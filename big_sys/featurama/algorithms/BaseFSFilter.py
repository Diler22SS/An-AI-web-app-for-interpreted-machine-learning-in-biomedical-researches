...
class BaseFeatureSelectorFilter(ABC):
    def __init__(self, pipeline: models.Model):
        self.pipeline = pipeline
        self._validate_input()
        self.params = self.pipeline.fs_filter_parameters
    ...
    def _validate_input(self) -> None:
        ...
        if not self.pipeline.preliminarily_selected_features:
            raise FeatureSelectionError("Empty feature list")

        if not self.pipeline.fs_filter:
            raise FeatureSelectionError("Wasn't choosen filter method")

        if not self.pipeline.fs_filter_parameters:
            raise FeatureSelectionError("Empty filter method's params")

    def _get_features_data(self) -> pd.DataFrame:
        """Get DataFrame with only preliminarily selected features"""
        df = self.pipeline.get_dataframe()
        return df[self.pipeline.preliminarily_selected_features]
    ...
    def _save_result(self, selected_features: list) -> None:
        """Save results"""
        self.pipeline.fs_filter_selected_features = selected_features
        self.pipeline.save()
